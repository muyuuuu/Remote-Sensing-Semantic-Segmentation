...
def write_mask(mask, path, output_dir):
    ...
    output_path = os.path.join(output_dir, region,
                               f"{filename}_prediction.tif")
...
def main(log_dir, output_directory, device):
    ...
    step = 512
    start_point = [0, 372, 744, 1116, 1488]
    for i, batch in tqdm(enumerate(dataloader), total=len(dataloader)):
        # 裁剪与拼接
        x = batch["image"].to(device)
        h, w = x.shape[-2:]
        mask = torch.zeros(1, 16, 2048, 2048).cuda()
        x = pad(x)
        for row in start_point:
            for col in start_point:
                tmp = task(x[:, :, row:row + step, col:col + step])
                mask[:, :, row:row + step, col:col + step] += tmp
        mask = mask[0, :, :h, :w]
